        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    var isAdmin = await UserManager.IsInRoleAsync(user.Id, Enums.RoleType.Administrator.ToString());
                    if (isAdmin)
                    {
                        if (user.Status != Enums.UserStatus.Active.ToString())
                        {
                            ModelState.AddModelError("", "Your account is not active, please contact admin.");
                            return View(model);
                        }
